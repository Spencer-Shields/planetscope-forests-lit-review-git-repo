...
j_df = df %>%
  mutate(Publication.Title = toupper(Publication.Title)) %>%
...
#histograms
{
  n_dates_hist_df = temp_res_df %>%
    group_by(n_timeseries_dates) %>%
    summarise(count = n())
  
  bitemp_vs_no_summ = temp_res_df %>%
    mutate(bitemp_or_multitemp = case_when(
      n_timeseries_dates <= 2 ~ 'bitemporal',
      .default = 'multitemporal'
    )) %>%
    group_by(bitemp_or_multitemp) %>%
    summarise(count = n())
  
  ggplot(bitemp_vs_no_summ, aes(x = bitemp_or_multitemp, y = count))+
    geom_col()
  
  n_dates_breaks = c(min(temp_res_df$n_timeseries_dates) - 1, 2, max(temp_res_df$n_timeseries_dates) + 1)
  ndh = hist(x = temp_res_df$n_timeseries_dates, breaks = n_dates_breaks, nclass = 2)
  
  
  ggplot(n_dates_hist_df, aes(x = n_timeseries_dates, y = count))+
    geom_point()+
